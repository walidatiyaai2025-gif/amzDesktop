...
class RemoteScreenApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      title: 'Remote Screen Mobile V2.4',
...
class _ConnectPageState extends State<ConnectPage> {
  ...
  Future<void> _connect() async {
    ...
    if (base.isEmpty || token.length != 6) {
      setState(() => _error =
          'Enter the exact PC address shown by V2.4 and the 6-digit session code.');
      return;
    }
    ...
    try {
      ...
      setState(() => _error =
          'Cannot reach the PC. Use the exact IP and port displayed by Remote Screen Desktop V2.4.');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Remote Screen Mobile V2.4')),
      body: SafeArea(
        child: Center(
