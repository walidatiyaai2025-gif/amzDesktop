...
sealed class RemoteServer : IDisposable
{
    ...
    public string BackendName => capture?.Name ?? "Unavailable";
    public int CaptureWidth => capture?.Width ?? 0;
    public int CaptureHeight => capture?.Height ?? 0;
    public bool AudioReady => audio.IsReady;
    public string Diagnostics { get; private set; } = "";

    public RemoteServer(CaptureBackendMode requestedMode)
    {
    ...
    public void Start()
    {
        capture = CaptureBackendFactory.Create(requestedMode, out var captureDiag);
        ...
        if (!audio.IsReady && !string.IsNullOrWhiteSpace(audio.LastError))
        {
    ...
    async Task StreamScreenAsync(NetworkStream stream)
    {
        ...
        while (!cts.IsCancellationRequested)
        {
            try
            {
                var jpeg = capture!.CaptureJpeg(68);
            ...
            {
                Diagnostics = $"Capture runtime error: {ex.Message}";
